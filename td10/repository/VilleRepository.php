...
class VilleRepository extends ItemsRepository
{
    ...
    {
        $connection = $this->getConnection();
        $query = "INSERT INTO ville (label, id_pays) VALUES (:label, :id_pays)";
        $statement = $connection->prepare($query);
        $label = $object->getLabel();
        $id_pays = $object->getIdPays();
    ...
    {
        $connection = $this->getConnection();
        $query = "UPDATE ville SET label = :label, id_pays = :id_pays WHERE id = :id";
        $statement = $connection->prepare($query);
        $statement->bindParam(':id', $object->getId(), PDO::PARAM_INT);
        $statement->bindParam(':label', $object->getLabel(), PDO::PARAM_STR);
        $statement->bindParam(':id_pays', $object->getIdPays(), PDO::PARAM_INT);
        return $statement->execute();
    }
    ...
    {
        $connection = $this->getConnection();
        $query = "SELECT id FROM ville WHERE name = :label";
        $statement = $connection->prepare($query);
        $statement->bindParam(':label', $label, PDO::PARAM_STR);
        $statement->execute();
    ...
    function findByIdPays($pays_id) {
        $connection = $this->getConnection();
        $query = "SELECT * FROM ville WHERE id_pays = :id_pays";
        $statement = $connection->prepare($query);
        $statement->bindParam(':id_pays', $pays_id, PDO::PARAM_INT);
        $statement->execute();
        return $statement->fetchAll(PDO::FETCH_ASSOC);
    }
